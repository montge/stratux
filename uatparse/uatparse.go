...
func (f *UATFrame) decodeAirmet() {
	...
	fmt.Fprintf(io.Discard, "record_format=%d\n", record_format)
	product_version := (uint8(f.FISB_data[0]) & 0x0F)
	fmt.Fprintf(io.Discard, "product_version=%d\n", product_version)
	record_count := (uint8(f.FISB_data[1]) & 0xF0) >> 4
	fmt.Fprintf(io.Discard, "record_count=%d\n", record_count)
	...
	fmt.Fprintf(io.Discard, "location_identifier=%s\n", location_identifier)
	...
	fmt.Fprintf(io.Discard, "record_reference=%d\n", record_reference)
	...
	if record_count != 1 {
		fmt.Fprintf(io.Discard, "record_count=%d, != 1\n", record_count)
		return
	}
	/*
		0 - No data
	...
	*/
	...
	case 2:
		...
		if len(f.FISB_data)-int(record_length) < 6 {
			fmt.Fprintf(io.Discard, "FISB record not long enough: record_length=%d, len(f.FISB_data)=%d\n", record_length, len(f.FISB_data))
			return
		}
		fmt.Fprintf(io.Discard, "record_length=%d\n", record_length)
	...
	case 8:
		...
		fmt.Fprintf(io.Discard, "overlay_record_identifier=%d\n", overlay_record_identifier)
		object_label_flag := uint8(record_data[4] & 0x01)
		fmt.Fprintf(io.Discard, "object_label_flag=%d\n", object_label_flag)

		if object_label_flag == 0 { // Numeric index.
			object_label := (uint16(record_data[5]) << 8) | uint16(record_data[6])
			record_data = record_data[7:]
			fmt.Fprintf(io.Discard, "object_label=%d\n", object_label)
		} else {
			object_label := dlac_decode(record_data[5:], 9)
			record_data = record_data[14:]
			fmt.Fprintf(io.Discard, "object_label=%s\n", object_label)
		}

		element_flag := (uint8(record_data[0]) & 0x80) >> 7
		fmt.Fprintf(io.Discard, "element_flag=%d\n", element_flag)
		qualifier_flag := (uint8(record_data[0]) & 0x40) >> 6
		fmt.Fprintf(io.Discard, "qualifier_flag=%d\n", qualifier_flag)
		param_flag := (uint8(record_data[0]) & 0x20) >> 5
		fmt.Fprintf(io.Discard, "param_flag=%d\n", param_flag)
		object_element := uint8(record_data[0]) & 0x1F
		fmt.Fprintf(io.Discard, "object_element=%d\n", object_element)

		object_type := (uint8(record_data[1]) & 0xF0) >> 4
		fmt.Fprintf(io.Discard, "object_type=%d\n", object_type)

		object_status := uint8(record_data[1]) & 0x0F
		fmt.Fprintf(io.Discard, "object_status=%d\n", object_status)
		...
		if qualifier_flag == 0 { //TODO: Check.
			...
			fmt.Fprintf(io.Discard, "object_qualifier=%d\n", object_qualifier)
			fmt.Fprintf(io.Discard, "%02x%02x%02x\n", record_data[2], record_data[3], record_data[4])
		...
		case 3: // Extended Range 3D Polygon (MSL).
			...
			for i := 0; i < int(overlay_vertices_count); i++ {
				...
				fmt.Fprintf(io.Discard, "lat_raw=%d, lng_raw=%d, alt_raw=%d\n", lat_raw, lng_raw, alt_raw)
				lat, lng := airmetLatLng(lat_raw, lng_raw, false)

				alt := alt_raw * 100
				fmt.Fprintf(io.Discard, "lat=%f,lng=%f,alt=%d\n", lat, lng, alt)
				fmt.Fprintf(io.Discard, "coord:%f,%f\n", lat, lng)
				var point GeoPoint
				point.Lat = lat
				point.Lon = lng
				point.Alt = alt
		...
		case 9: // Extended Range 3D Point (AGL). p.47.
			if len(record_data) < 6 {
				fmt.Fprintf(io.Discard, "invalid data: Extended Range 3D Point. Should be 6 bytes; %d seen.\n", len(record_data))
			} else {
		...
		case 7, 8: // Extended Range Circular Prism (7 = MSL, 8 = AGL)
			if len(record_data) < 14 {
				fmt.Fprintf(io.Discard, "invalid data: Extended Range Circular Prism. Should be 14 bytes; %d seen.\n", len(record_data))
			} else {
				...
				fmt.Fprintf(io.Discard, "lat_bot, lng_bot = %f, %f\n", lat_bot, lng_bot)
				fmt.Fprintf(io.Discard, "lat_top, lng_top = %f, %f\n", lat_top, lng_top)

				if geometry_overlay_options == 8 {
					fmt.Fprintf(io.Discard, "alt_bot, alt_top = %d AGL, %d AGL\n", alt_bot, alt_top)
				} else {
					fmt.Fprintf(io.Discard, "alt_bot, alt_top = %d MSL, %d MSL\n", alt_bot, alt_top)
				}
				fmt.Fprintf(io.Discard, "r_lng, r_lat = %f, %f\n", r_lng, r_lat)

				fmt.Fprintf(io.Discard, "alpha=%d\n", alpha)
			}
		default:
			fmt.Fprintf(io.Discard, "unknown geometry: %d\n", geometry_overlay_options)
		}
	//case 1: // Unformatted ASCII Text.
	default:
		fmt.Fprintf(io.Discard, "unknown record format: %d\n", record_format)
	}
...
func (f *UATFrame) decodeInfoFrame() {
	...
	default:
		fmt.Fprintf(io.Discard, "don't know what to do with product id: %d\n", f.Product_id)
	}
